...
const WhyIdeaHub = () => {
    return (
        <section className="container mx-auto mt-5 px-8 py-5 md:mt-8 text-slate-950 dark:text-slate-100">
            <h2 className="mb-5 text-2xl font-bold text-slate-950 dark:text-white md:text-3xl">
                Why IdeaVault?
            </h2>
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
                {features.map(({ title, description, icon: Icon }) => (
                    <article
                        key={title}
                        className="flex items-start gap-4 rounded-lg bg-white dark:bg-slate-900 p-5 shadow-md"
                    >
